...
// src/config/db.js
// PlanetScale requires SSL connections. The ssl option below handles this.
// In development (local MySQL), SSL is disabled. In production, it's required.
...
const pool = mysql.createPool({
  ...
  // SSL is required for PlanetScale in production.
  // In development, DB_SSL is not set so this is undefined (no SSL).
  // ssl: { rejectUnauthorized: true } = verify the server's certificate.
  ...(env.db.ssl ? {
    ssl: {
      rejectUnauthorized: true,
      // PlanetScale provides their own cert authority. Setting this to true
      // tells Node.js to use its built-in trusted certificate store.
    }
  } : {}),
});
...
async function withTransaction(fn, maxRetries = 3) {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    ...
    try {
      ...
      await connection.rollback();
      const isDeadlock    = err.errno === 1213;
      const isLockTimeout = err.errno === 1205;
      if ((isDeadlock || isLockTimeout) && attempt < maxRetries) {
        lastError = err;
        logger.warn('[DB] Deadlock detected, retrying', { attempt, errno: err.errno });
        await new Promise(r => setTimeout(r, attempt * 100));
        continue;
      }
      throw err;
