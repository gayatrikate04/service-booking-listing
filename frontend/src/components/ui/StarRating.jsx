...
export function StarRating({ rating, size = 'sm', showValue = false }) {
  ...
  <span className="inline-flex items-center gap-1 max-w-full overflow-hidden">
    ...
    <span
      className={`${sizes[size]} text-amber-400 flex flex-wrap items-center leading-none`}
    >
      {[1, 2, 3, 4, 5].map((i) => (
        <span
          key={i}
    ...
    {showValue && (
      <span
        className={`${sizes[size]} font-medium text-gray-900 break-words whitespace-nowrap`}
      >
